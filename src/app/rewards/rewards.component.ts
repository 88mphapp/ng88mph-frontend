...
export class RewardsComponent implements OnInit {
  ...
  async loadData(loadUser: boolean, loadGlobal: boolean) {
    ...
    if (loadUser) {
      mph.methods.balanceOf(address).call().then(unstakedMPHBalance => {
    ...
    if (loadGlobal) {
      ...
      const rewardPerSecond = rewardsAmount.div(rewardsEndDate.minus(rewardsStartDate)).times(this.mphPriceUSD);
      let secondROI = rewardPerSecond.div(this.xMPHTotalSupply.times(this.xMPHPriceUSD)).times(100);
      if (secondROI.isNaN()) {
  ...
  async loadRewardAccumulationStats() {
    ...
    Promise.all(aavePools.map(async poolInfo => {
      ...
      const stkaavePriceUSD = await this.helpers.getTokenPriceUSD(this.constants.AAVE);
