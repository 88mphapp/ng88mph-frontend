...
@Component({
  ...
  templateUrl: './modal-fractionalize.component.html',
  styleUrls: ['./modal-fractionalize.component.css']
})
export class ModalFractionalizeComponent implements OnInit {
  ...
  fractionalize() {
    ...
    const func = pool.methods.fractionalize(this.userDeposit.nftID, this.userDeposit.fundingID);
  ...
  earlyFractionalize() {
    ...
    const mphMinter = this.contract.getNamedContractAddress('MPHMinter');
    const mphAmount = this.helpers.processWeb3Number(this.mphTakeBackAmount.times(this.constants.PRECISION));
    const func = pool.methods.earlyFractionalize(this.userDeposit.nftID, this.userDeposit.fundingID);
