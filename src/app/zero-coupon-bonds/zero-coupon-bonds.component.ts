...
export class ZeroCouponBondsComponent implements OnInit {
  ...
  loadData(loadUser: boolean, loadGlobal: boolean): void {
    if (loadGlobal) {
      ...
      this.allPoolList = zcbPoolNameList.map((poolName) =>
        this.contract.getPoolInfo(poolName)
      );
      this.selectPool(0);
    }
  }
  ...
  selectPool(poolIdx: number) {
    ...
    this.selectedPoolZCBList = this.contract
      .getZeroCouponBondPool(this.selectedPoolInfo.name)
      .map((zcbInfo) => {
        return {
          zcbInfo,
    ...
    Promise.all(
      this.selectedPoolZCBList.map(async (zcbEntry) => {
        const zcbContract = this.contract.getZeroCouponBondContract(
          zcbEntry.zcbInfo.address,
          readonlyWeb3
        );
        const tokenDecimals = this.selectedPoolInfo.stablecoinDecimals;
        const tokenPrecision = Math.pow(10, tokenDecimals);
        zcbEntry.priceInUSD = await this.getZeroCouponBondPriceUSD(
          zcbEntry.zcbInfo
        );
        zcbEntry.totalSupply = new BigNumber(
          await zcbContract.methods.totalSupply().call()
        ).div(tokenPrecision);
        zcbEntry.totalSupplyUSD = zcbEntry.priceInUSD.times(
          zcbEntry.totalSupply
        );
        zcbEntry.maturationTimestamp = new Date(
          +(await zcbContract.methods.maturationTimestamp().call()) * 1e3
        );
        if (this.wallet.connected && !this.wallet.watching) {
          zcbEntry.userBalance = new BigNumber(
            await zcbContract.methods.balanceOf(this.wallet.userAddress).call()
          ).div(tokenPrecision);
          zcbEntry.userBalanceUSD = zcbEntry.priceInUSD.times(
            zcbEntry.userBalance
          );
        } else if (this.wallet.watching) {
          zcbEntry.userBalance = new BigNumber(
            await zcbContract.methods
              .balanceOf(this.wallet.watchedAddress)
              .call()
          ).div(tokenPrecision);
          zcbEntry.userBalanceUSD = zcbEntry.priceInUSD.times(
            zcbEntry.userBalance
        ...
        zcbEntry.impliedInterestRate = await this.computeImpliedInterestRate(
          zcbEntry
        );
  ...
  openZCBModal(zcbEntry: ZeroCouponBondTableEntry) {
    const modalRef = this.modalService.open(ModalZeroCouponBondInfoComponent, {
  ...
  async computeImpliedInterestRate(
    zcbEntry: ZeroCouponBondTableEntry
  ): Promise<BigNumber> {
    const underlyingPrice = await this.helpers.getTokenPriceUSD(
      this.selectedPoolInfo.stablecoin
    );
    const roi = new BigNumber(underlyingPrice)
      .div(zcbEntry.priceInUSD)
      .minus(1);
    const secondsToMaturation =
      (zcbEntry.maturationTimestamp.getTime() - this.now.getTime()) / 1e3;
    if (secondsToMaturation <= 0) {
      // already mature
      return new BigNumber(NaN);
    }
    const apy = roi
      .div(secondsToMaturation)
