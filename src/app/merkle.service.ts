...
export class MerkleService {
  ...
  getETHClaimForAddress(address: string) {
    const merkleTree =
      require('src/assets/json/eth-snapshot-merkle-tree.json') as MerkleDistributorInfo;
    const checksumAddress = this.wallet
      .readonlyWeb3()
  ...
  getMPHClaimForAddress(address: string) {
    const merkleTree =
      require('src/assets/json/mph-snapshot-merkle-tree.json') as MerkleDistributorInfo;
    const checksumAddress = this.wallet
      .readonlyWeb3()
      .utils.toChecksumAddress(address);
    const claim = merkleTree.claims[checksumAddress];
